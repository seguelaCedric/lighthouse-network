...
async function test() {
  ...
  // Exact same payload as server action
  const testPhone = `+33612399888`;

  // This is exactly what actions.ts does:
  type SyncPayload = Parameters<typeof syncCandidateUpdate>[1];
  const syncPayload: SyncPayload = {
    first_name: 'CedricTest',
    last_name: 'SeguelaTest',
  };
  // Only add optional fields if they are defined
  syncPayload.phone = testPhone;

  console.log('\n=== SYNC PAYLOAD ===');
  console.log('Keys:', Object.keys(syncPayload));
  console.log('phone in syncPayload:', 'phone' in syncPayload);
  console.log('syncPayload.phone:', syncPayload.phone);
